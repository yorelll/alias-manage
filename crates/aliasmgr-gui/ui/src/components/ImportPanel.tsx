...
export function ImportPanel() {
  ...
  return <section className="panel"><h1>Import</h1><input value={file} onChange={(event) => setFile(event.target.value)} placeholder="Path to JSON/TOML" /><button onClick={preview}>Preview</button>{error && <p role="alert">{error}</p>}{report && <pre>{JSON.stringify(report, null, 2)}</pre>}<button disabled={!report} onClick={confirm}>Confirm import</button></section>;
}
