...
test("prerelease.yml: Linux GUI apt-source fix verifies no azure.archive.ubuntu.com remains", async () => {
  ...
  assert.match(
    src,
    /azure\.archive\.ubuntu\.com.*still present|ERROR.*azure\.archive\.ubuntu\.com|azure\.archive\.ubuntu\.com remaining|no azure\.archive\.ubuntu\.com remaining|azure\.archive\.ubuntu\.com endpoint remains/,
