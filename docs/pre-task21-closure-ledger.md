...
| SQLite sync transaction/forward recovery | CI-verifiable | sync/storage tests | Per-Shell `shell_state` success/failure persists in SQLite; prepared recovery remains green; fast CI `32017853467` and integration `32017999024` passed. Transaction binding, committed forward recovery, and crash injection remain incomplete |
