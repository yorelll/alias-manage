...
|---|---|---|---|
| GUI npm cache | implementation-feasible | lockfile + pinned GUI CI cache | `crates/aliasmgr-gui/ui/package-lock.json` committed and CI uses `npm ci`/npm cache; fast CI `32089342208` passed |
...
| Windows ACL writable path | environment-blocked | Windows native ACL test + manual acceptance | Conservative `Unknown` result is implemented; no verified native ACL boundary is available, so this remains blocked rather than unchecked implementation debt |
...
| Tombstone fingerprint/override recovery | CI-verifiable | Shell integration tests | Generated metadata carries tombstone names/deterministic fingerprints; CLI sync includes retired names; conditional session-cleanup and override recoverability tests pass fast CI `32016526640` and integration `32017482835`. Full live-definition capture/skip wiring remains incomplete |
