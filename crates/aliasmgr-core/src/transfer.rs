...
pub fn import_json(path: &Path, existing: &BTreeMap<String, AliasRecord>, strategy: ConflictStrategy) -> Result<ImportReport, AliasError> {
    ...
    if file.format_version != FORMAT_VERSION { return Err(AliasError::Config(format!("unsupported format_version: {}", file.format_version))); }
    let mut report = ImportReport { imported: vec![], skipped: vec![], warnings: vec![], unsupported: vec![] };
    for mut alias in file.aliases {
        ...
        report.imported.push(alias.name);
    }
    Ok(report)
}
