...
fn main() {
    ...
    let result = match &cli.command {
        cli::Command::Add(args) => commands::add(cli.config_dir.as_deref(), args).map(|_| messages::ALIAS_ADDED),
        ...
        cli::Command::Remove { name, .. } => commands::remove(cli.config_dir.as_deref(), name).map(|_| messages::ALIAS_REMOVED),
        cli::Command::Enable { name } => commands::enable(cli.config_dir.as_deref(), name, true).map(|_| messages::ALIAS_ADDED),
        cli::Command::Disable { name } => commands::enable(cli.config_dir.as_deref(), name, false).map(|_| messages::ALIAS_REMOVED),
